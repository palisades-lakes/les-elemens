...
$\f:\vspace \mapsto \Space{W}$,
at a point $\v$,
is as the linear transformation $\Lmap:\vspace \mapsto \Space{W}$,
...
with respect to some bases for $\vspace$ and $\Space{W}$.
...
\sum_{i=0}^{m-1}
\sum_{j=0}^{n-1}
\db{j}{\f_i}{\u} \left( \e_i^{\Space{W}} \otimes \e_j^{\vspace} \right)
\end{equation}
...
Suppose
$\f:\Space{U} \mapsto \vspace$,
$\g:\vspace \mapsto \Space{W}$,
and
...
A map $\f(\v_0 \ldots \v_k):\vspace_0 \oplus \ldots \oplus \vspace_k \mapsto \Space{W}$
...
A map $\f(\v,\u):\vspace_0 \oplus \vspace_1 \mapsto \Space{W}$
is {\it bilinear} if
\begin{eqnarray}
...
$\{ \Lmap : \vspace \mapsto \Space{W} \}$
is itself a inner product space $\Space{L}(\vspace,\Space{W})$,
with the inner product defined by
$\Lmap \bullet \Mmap = \sum_{i=0}^{m-1} \sum_{j=0}^{n-1} \Lmap_{ij} \Mmap_{ij}$.
The set of linear maps
$\Emap_{ij}^{\Space{L}(\vspace,\Space{W})}  = \e_i^{\Space{W}} \otimes \e_j^{\vspace}$
are the canonical basis vectors for $\Space{L}(\vspace,\Space{W})$.

If $\f$ is a map between spaces of linear maps,
$\f : \Space{L}(\vspace_0,\Space{W}_0) \mapsto \Space{L}(\vspace_1,\Space{W}_1)$,
its derivative, $\Da{\f}$,
is a map from a space of linear maps
to a space of linear maps between two
spaces of linear maps:
$\Da{\f} : \Space{L}(\vspace_0,\Space{W}_0) \mapsto
\Space{L}(\Space{L}(\vspace_0,\Space{W}_0), \Space{L}(\vspace_1,\Space{W}_1))$.
...
of the $ij$-th canonical basis vector, $\Emap_{ij}^{\Space{L}(\vspace_0,\Space{W}_0)}$.
As usual the value of the partial derivative at a specific
$\Lmap_0 \in  \Space{L}(\vspace_0,\Space{W}_0)$,
$\db{ij}{\f}{\Lmap_0}$ is an element of the co-domain of $\f$,
a linear map in  $\Space{L}(\vspace_1,\Space{W}_1)$.
...
a linear map in $\Space{L}(\vspace_0,\Space{W}_0)$.
...
&&\db{ij}{\Transpose}{\Lmap} =  \e_j^{\vspace} \otimes \e_i^{\Space{W}}
\forall \Lmap
\\
...
Let
$\f : \Space{L}(\vspace_0,\Space{W}_0) \mapsto \Space{L}(\vspace_1,\Space{W}_1)$,
$\g : \Space{L}(\vspace_0,\Space{W}_0) \mapsto \Space{L}(\Space{U}_1,\vspace_1)$,
and
$\h = \f\g : \Space{L}(\vspace_0,\Space{W}_0) \mapsto \Space{L}(\Space{U}_1,\Space{W}_1)$.
Note that
$\db{ij}{\f}{\Lmap} \in  \Space{L}(\vspace_1,\Space{W}_1)$,
$\db{ij}{\g}{\Lmap} \in  \Space{L}(\Space{U}_1,\vspace_1)$,
and
...
: \Space{L}(\vspace,\Space{W}) \mapsto \Space{L}(\vspace,\vspace)$.
...
\db{ij}{\LTL}{\Lmap}
=
\left( \e_j^{\vspace} \otimes \e_i^{\Space{W}} \right) \Lmap
+
\Lmap^{\dagger} \left( \e_i^{\Space{W}} \otimes \e_j^{\vspace} \right)
=
...
\left( \e_i^{\Space{W}} \otimes \e_j^{\vspace} \right)
\\
& = &
\sum_{ij}
...
\sum_{i}
\left(
\Lmap_{ik}  \e_i^{\Space{W}} \otimes \e_l^{\vspace}
\right)
+
\sum_{i}
\left(
\Lmap_{il}  \e_i^{\Space{W}} \otimes \e_k^{\vspace}
\right)
\nonumber
\\
& = &
...
A map $\Lmap : \vspace \mapsto \Space{W}$ is {\it invertible}
if, for all $\w \in \Space{W}$, there exists a $\v$ such that
$\w = \Lmap \v$.
In any reasonable topology,
the set of invertible linear maps $\vspace \mapsto \Space{W}$
...
\db{ij}{\Inverse()}{\Lmap}
 =
\lim_{ h \mapsto 0}
\frac{ \left( \Lmap + h (\e_i^{\Space{W}} \otimes \e_j^{\vspace}) \right)^{-1} - \Lmap^{-1} }{h}
\end{displaymath}
Note that
\begin{displaymath}
\Lmap + h (\e_i^{\Space{W}} \otimes \e_j^{\vspace})
 =
\left( \Identity^{\Space{W}} - ( -h ( \e_i^{\Space{W}} \otimes \e_j^{\vspace} )) \Lmap^{-1} \right) \Lmap
\end{displaymath}
and
\begin{eqnarray*}
\left( \Lmap + h (\e_i^{\Space{W}} \otimes \e_j^{\vspace}) \right)^{-1}
& = &
\Lmap^{-1} \left( \Identity^{\Space{W}} - ( -h )( \e_i^{\Space{W}} \otimes \e_j^{\vspace} ) \Lmap^{-1} \right)^{-1}
\\
& = &
\Lmap^{-1} \sum_{k=0}^{\infty} \left( -h ( \e_i^{\Space{W}} \otimes \e_j^{\vspace} ) \Lmap^{-1} \right)^{k}
\nonumber
\end{eqnarray*}
Therefore
\begin{displaymath}
\frac{ \left( \Lmap + h (\e_i^{\Space{W}} \otimes \e_j^{\vspace}) \right)^{-1} - \Lmap^{-1} }{h}
 =
- \Lmap^{-1} ( \e_i^{\Space{W}} \otimes \e_j^{\vspace} )  \Lmap^{-1} + O(h)
\end{displaymath}
which implies
\begin{equation}
\da{ij}{\Lmap^{-1}}
...
\left[
\left( \Lmap^{\dagger} \Lmap \right)^{-1}
\left( \e_j^{\vspace} \otimes \e_i^{\Space{W}} \right)
\right]
\nonumber
\end{eqnarray}
...
a linear map from $\Space{L}(\vspace,\Space{W}) \mapsto \Space{L}(\vspace,\vspace)$.
...
\right]
$,
an element of $\Space{L}(\vspace,\vspace)$
and
$
\left[
\left( \c_k^{\Lmap} \otimes \e_l^{\vspace} \right)
...
\right]
$,
an element of $\Space{L}(\vspace,\Space{W})$.

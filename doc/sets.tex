...
\item[Specification] 
$\Set{S} =
\{ u \glssymbol{elementOf} \Set{U} | p(u) = \text{true} \}$ 
...
\item[Enumeration]
$\Set{S} =
\{ f(u) | u \glssymbol{elementOf} \Set{U} \}$ 
...
Families: $\{ \Set{S}_{i} \, | \, i \in \, \text{index set} \, \Set{I}
\}$.
...
$\bigcap_{i \in \Set{I}} \Set{S}_{i} = \{ s | s \in \Set{S}_{i}
\forall \Set{S}_{i} \}$
...
$\Set{A} \times \Set{B} =
\{[a \, b] | a \in \Set{A} \text{ and } b \in \Set{B}\}$.

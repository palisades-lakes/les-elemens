...
Let $\Vector{v}, \Vector{w} \in \vspace$.
...
Let $\Vector{v}, \Vector{u} \in \vspace, \Vector{w} \in \Space{W}.$
$\Vector{w} \otimes \Vector{v}$ is a rank 1 linear map
from $\vspace$ to $\Space{W}$, defined by:
\begin{equation}
...
(eg. $\Space{L}space(\vspace,\Space{W})$, the space of linear maps
...
If $\Vector{t} \in \Space{T}$, $\Vector{u}, \Vector{v} \in \vspace$, 

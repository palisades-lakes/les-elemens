...
\ldots There is a tendency in much contemporary literature to present material
...
Author seems to think that general equivalence relations
are the key idea and something new to category theory, 
or maybe infinity category theory. 
At best, this is a misunderstanding---there's nothing 
in the article that explains how category theory's version of
equivalence relations is any different from the old usual version.
And, at least at my state of knowledge, I can't imagine they
are referring to.}
...
to the uninitiated.
Examples: product/sum, epi/monomorphisms, {\ldots}.
...
and what the $\mathsf{compose}$ operation does in those cases.
...
eg, ``left-cancelable'' rather than ``mono-morphism'',
...
$V_0 = \mathsf{tail}(e)$ and $V_1 = \mathsf{head}(e)$
...
orientation from $90^{\circ}$ turn of primal edge.~\cite{wiki_2020_dual_graph})
...
on the graph:
\begin{description}
\item[$\mathsf{compose}$ \textrm{(infix operator: $\circ$)}]\mbox{}\\
...
is really necessary.
\\
For example, we could extend $\mathsf{compose}$ 
...
Are $\mathsf{compose}$ and the resulting edges
used for anything other than defining path equivalence?)
This suggests replacing $\mathsf{compose}$ with 
...
In this case, the $\mathsf{value}$ function maps each path
...
whenever convenient.
The main operation on paths is $\mathsf{concatenate}$
(infix operator: $\diamond$).
With paths $p_0,p_1,p_2$,
assuming $\mathsf{head}(p_0) = \mathsf{tail}(p_1)$: 
\begin{align*}
p_2 & = \mathsf{concatenate}(p_0,p_1) = p_0 \diamond p_1
\end{align*}
\textbf{Note:} $\mathsf{concatenate}$ reverses the arguments
of $\mathsf{compose}$. 
We start at $\mathsf{tail}(p_0)$ and end at 
$\mathsf{head}(p_1)$.
...
a path evaluation function: $\left[G, \mathsf{value}\right]$.
Two paths are \textit{equivalent} ($\simeq$) 
if they have the same 
$\mathsf{head}$, $\mathsf{tail}$, and $\mathsf{value}$:
\[
\left[ p_0 \simeq \p_1 \right]
\;  \Leftrightarrow \;
\left[
\left( \mathsf{head}(p_0) = \mathsf{head}(p_1) \right)
\wedge
\left( \mathsf{tail}(p_0) = \mathsf{tail}(p_1) \right)
\wedge
\left( \mathsf{value}(p_0) = \mathsf{value}(p_1) \right)
\right]
\]
\end{definition}
...
needed. 
The $\mathsf{value}$  function is not shown,
but that matches typical
use of diagrams, which omit the implied
composition and identity edges.
Note that $\mathsf{identity}$ is not needed at all;
the constraints between $\mathsf{identity}$ and $\mathsf{compose}$
are dropped; and $\mathsf{concatenate}$ is associative by definition.
...
path $\mathsf{value}$ function.
It is $\mathsf{compose}$ for general paths, and, 
for single edge identity loop paths,
it returns the 
path consisting of the single 
$\mathsf{head}$/$\mathsf{tail}$ vertex of the loop.
...
& V_2 
\end{tikzcd}
\caption{Mono edge $e$ if diagram commutes.}
\label{fig:mono}
\end{figure}
...
& V_2 
\end{tikzcd}
\caption{Epi edge $e$ if diagram commutes.}
\label{fig:epi}
\end{figure}
...
(\textbf{TODO:} mono vs monic, epi vs epic)
...
An edge $e$ is \textit{epi} (aka epimorphism)
if, for any parallel edges $f,g$ departing $\mathsf{head}(e)$:
\[
\left( f \circ e = g \circ e \right) \Rightarrow (f = g)
\]

Mono and epi definitions are equivalent to one with the parallel
edges replaced by parallel paths. (Is a proof needed here?)
...
{\sloppy
An edge $e$ is \textit{split mono} (aka has a left inverse)
if there exists an $e^{-\text{left}}$, the \textit{retraction},
such that
\[
e^{-\text{left}} \circ e = \mathsf{identity}(\mathsf{tail}(e))
\]
Every split mono edge is mono.
}

{\sloppy
An edge $e$ is \textit{split epi} (aka has a right inverse)
if there exists an $e^{-\text{right}}$, the \textit{splitting}, 
such that
\[
e \circ e^{-\text{right}} = \mathsf{identity}(\mathsf{head}(e))
\]
Every split epi edge is epi.
}

(\textbf{Question:} do split mono/epi definitions work if we replace
edge $e$ by path $p$?)

An edge $e$ is \textit{iso} (aka an isomorphism, invertible)
...
\]
In other words, $e$ is both split mono and split epi,
and the retraction and splitting edges are the same.
(Note that this seems much simpler in left/right invertible language.)

(\textbf{Question:} can we define isomorphisms via paths?)

(\textbf{Note:} in a topos mono$+$epi $\Rightarrow$ iso.
Is that because all mono/epi are split?)

(\textbf{TODO:} describe these ideas in the context of
structures and structure-preserving functions vs
set and all functions, to distinguish epi/mono from split versions.
eg non-surjective epimorphisms, noon-injective monomorphism.)

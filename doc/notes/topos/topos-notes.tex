...
\tcblower
{Weyl, 
\emph{Die heutige Erkenntnislage in der Mathematik}~\cite{weyl1926heutige};
...
\tcblower
{Goldblatt, \emph{Topoi, Preface}~\cite{goldblatt_1984_topoi}}
\end{boxquote}
...
\tcblower
{MacLane, \emph{Of course and courses}~\cite{maclane1954}}
\end{boxquote}
...
\tcblower
{Asperi and Longo, \emph{Categories, Types, and Structures: 
...
for the theories.
\tcblower
{Barr and Wells, \emph{Category theory for computing science}~\cite{barr_wells_2020}}
\end{boxquote}
...
\tcblower
{Geroch, \emph{Mathematical Physics}~\cite{geroch_1985}}
\end{boxquote}
...
\tcblower
{Leinster, \emph{Basic Category Theory}~\cite{leinster_2016_basic_category_theory}}
\end{boxquote}
...
that we do.
\tcblower
{Thurston, \emph{On proof and progress in mathematics}~\cite{thurston_1994_proof}}
\end{boxquote}
...
A \emph{directed (multi)graph} (aka digraph) is
an ordered pair $G=\left( \Set{V}, \Set{E} \right)$,
where $\Set{V}$ is a set of \emph{vertices}, 
$\Set{E}$ a set of \emph{edges}, ordered pairs of vertices.
...
A graph may be called a \emph{multigraph} when $1$ is
not assumed.

In a context where loops are allowed,
a graph with no loops may be called \emph{simple}.
...
A \emph{path} is a sequence of edges where the head of each
edge matches the tail of the following edge, if there is one.

An \emph{directed acyclic graph} (aka DAG) has no paths
connecting a vertex to itself.

A directed graph is \emph{(strongly) connected} if there is a path
from any vertex to any other (weakly if there is path
ignoring direction.

(\textbf:{TODO:} straighten out \emph{path} vs \emph{walk} vs
\emph{trail}; directed vs undirected; sequence of alternating
vertices and edges, starting and beginning with a vertex.)

A \emph{root} vertex (aka source) has no entering edges.

A \emph{leaf} vertex (aka sink) has no departing edges.

A \emph{tree} is a directed graph with one root, 
where all vertices have at most entering edge.

(Mesh) \emph{dual} of a graph interchanges the roles of edges 
...
A \emph{category} is a directed (multi) graph 
...
In this context, a \emph{path} is taken to be a sequence of
...
(Until I think of a better name\ldots)\\
A \emph{kategory} is a directed (multi) graph together with
a path evaluation function: $\left[G, \pvalue\right]$.
Two paths are \emph{equivalent} ($\simeq$) 
...
A \emph{functor} is simply a structure-preserving function
...
\begin{definition}[Structure-preserving function between directed
...
A \emph{functor} $F:C_0 \rightarrow C_1$ is 
...
\begin{definition}[Phunctor (kategory)]
(Very unlikely to stick with this name.)

A \emph{phunctor} $P:K_0 \rightarrow K_1$ is 
a structure-preserving function
...
% \begin{definition}[Diagram (pictorial)
% \textmd{\textcite{geroch_1985,goldblatt_1984_topoi,
% hillman_2001_cat_primer}}]
% ``By \emph{diagram} we mean any collection of objects
...
% \begin{definition}[Diagram (graph to graph) 
% \textmd{\textcite{barr_wells_2020}}]
% Let $I$ and $G$ be {[directed) graphs]}. 
% A \emph{diagram} in $G$ of shape $I$
% is a graph homomorphism $D : I \rightarrow G$ of graphs. 
% \end{definition}

% \begin{definition}[Diagram (graph to category) 
% \textmd{\textcite{asperti_longo_1991}}]
...
% \end{definition}
% 
% \begin{definition}[Diagram (category to category) \textmd{(broken)}]
% \label{def:diagram_cat_to_cat}
% Let $I$ and $A$ be categories. 
% A \emph{diagram} is a functor $D : I \rightarrow A$.
% $I$ is the \emph{shape} (aka \emph{scheme},
% \emph{index category}) of the diagram $D$.\\
...
% \begin{itemize}
% \item \textcite[p$193/524$]{adamek_herrlich_strecker_1990}
% \item \textcite[p$101/311$]{awodey_2010}
% \item \textcite[p$118/183$]{leinster_2016_basic_category_theory} 
% (commutative diagrams introduced on p$11$!)
% \item \textcite[p$38/240$]{riehl_2017_cath_in_context}
%  (earlier than most),
% \item \textcite[p$180/267$]{spivakd_2013_cath_for_scientists}
% \item \textcite[p$16/86$]{vanoosten_2002} (also relatively early)
% \end{itemize}
...
% \begin{definition}[Diagram (informal but 
% consistent)~\textmd{{\textcite[Definition 1.1.33]{perrone_2019_cath_notes}}})]
...
\begin{definition}[Diagram (this time for 
real)~\textmd{{\textcite[Definition 1.4.20]{perrone_2019_cath_notes}}})]
``Let $C$ be a category, and $I$ be a small category. A diagram in
$C$ of shape $I$ is a functor $I \rightarrow C$.''
\end{definition}

(\textcite[p$200/390$]{lawvere_schanuel_2009_conceptual_math} 
doesn't quite fit, due to vagueness; roughly
map from directed graph to category, 
not specifying structure-preserving.)

Undefined in:
\textcite{maclane_1998_cat_for_working_mathematician}.

%-----------------------------------------------------------------
\setcounter{currentlevel}{\value{baseSectionLevel}-1}
\levelstay{Commuting Diagram}
...
then the diagram is said to \emph{commute}.
...
Two edges are \emph{parallel} is they have the same head and tail.
Same for paths.
...
An edge $e$ is \emph{monic} (aka a monomorphism, left-cancelable)
...
An edge $e$ is \emph{epic} (aka epimorphism, right-cancelable)
if, for any matching parallel edges $f,g$:
\begin{align*}
...
An edge $e$ is \emph{split monic} (aka has a left inverse)
if there exists an $e^{-\text{left}}$, the \emph{retraction},
such that
\[
...
An edge $e$ is \emph{split epic} (aka has a right inverse)
if there exists an $e^{-\text{right}}$, the \emph{splitting}, 
such that
\[
...
An edge $e$ is \emph{isic} (aka an isomorphism, invertible)
...
An path $p$ is \emph{left-cancelable} (aka a monomorphism, monic)

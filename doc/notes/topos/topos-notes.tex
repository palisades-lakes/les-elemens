...
\arrow[dr, bend right, "e_{34} \comop e_{13}"' red, dashed, red]
\& 
V_2 
...
& V_2 
\end{tikzcd}
\caption{Monic edge $e$: diagram commutes \implies $f=g$.}
\label{fig:monic}
\end{figure}
...
& V_2 
\end{tikzcd}
\caption{Epic edge $e$: diagram commutes \implies $f=g$.}
\label{fig:epic}
\end{figure}
...
\end{tikzcd}
\caption{Isic edge $e$, if diagram commutes.}
\label{fig:isic}
\end{figure}

\setcounter{currentlevel}{\value{baseSectionLevel}-2}
\levelstay{Category definitions}

See, for example, \cite{perrone_2019_cath_notes}.

An edge $e$ is \textit{monic} (aka a monomorphism)
...
An edge $e$ is \textit{epic} (aka epimorphism)
if, for any matching parallel edges $f,g$:
\begin{align*}
...
(\textbf{Question:} do split monic/epic definitions work if we replace
edge $e$ by path $p$?)
...
\]
$e$ is isic \iff it is both split monic and split epic,
...
\setcounter{currentlevel}{\value{baseSectionLevel}-2}
\levelstay{Kategory definition}
Monic and epic definitions are equivalent to one with the parallel

...
The vertices can in principle be any set of distinguishable things.
In application, it is often best to keep them abstract,
as pure bits of identity with no other properties or behavior.
Then functions can be used to map abstract vertices and
edges to more concrete entities related to the application at hand.
...
It's common to assume ($1$) that the vertices in an edge must be 
distinct (no loop edges) and ($2$) there is at most one edge
in a graph connecting any $2$ vertices.
In a context where loops are allowed,
a graph with no loops may be called \textit{simple}.
A graph may be called a \textit{multigraph} when ($2$) is
not assumed.
(Special case of (oriented) simplicial complex.)
...
\setcounter{currentlevel}{\value{baseSectionLevel}-1}
\levelstay{Category}
\label{sec:Directed_graph_category}

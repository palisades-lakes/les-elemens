...
\item[(Data) Structure] (need as better name)
...
\begin{tikzcd}
V_0
\arrow[drr, bend left, "e_{02}"]
\arrow[ddr, bend right, "e_{03}"]
\arrow[dr, "{e_{01}}" ] & & \\
& V_1 \arrow[r, "e_{12}"] \arrow[d, "e_{13}"]
& V_2 \arrow[d, "e_{24}"] \\
& V_3 \arrow[r, "e_{34}"]
& V_4
\end{tikzcd}
...
then
$\mathsf{compose}(e_0,e_1) = e_{10}$ is defined
...
with $\mathsf{compose}(p) = e$ such that 
...
for directed graphs: for every path, there is a single edge
directly connecting its tail to its head.
I don't know of any non-categorical applications of directed graphs
that have this property.
\par
It's tempting to think that we could dispense with $\mathsf{compose}$
and just work directly with paths, but that's not simple, and
the reason will be clear soon, when we talk about commuting diagrams.
Commuting diagrams make assertions about the equivalence of certain
paths, and they make those by asserting that applying $\mathsf{compose}$
to each path returns the same edge.
\par
(\textbf{Question:} is there an alternative way to express this
equivalence without adding extra edges to the graph?)

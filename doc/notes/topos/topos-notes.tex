...
\cite{wiki:Abstract-nonsense-2020}
...
of a bj ects together with a binary operation satisfying such and
...
Anything in software complextity measures that would apply?
...
Is there something more limke ``sets and functions'' where
the standard constructs make sense?

Is this question related to concrete 
categories?~\cite{wiki:concrete-categroy-2020}
...
%-----------------------------------------------------------------
\setcounter{currentlevel}{\value{baseSectionLevel}}
\levelstay{Foundation Sketch}
\label{sec:Foundation-Sketch}
...
axiom of choice, non-measureable sets, {\ldots},
...
something like a dozen schools of intuitionist/constructive
mathematics, sometimes using different words to mean the
same thing, sometimes the same words to mean different things.

%-----------------------------------------------------------------
\setcounter{currentlevel}{\value{baseSectionLevel}-1}
\levelstay{What would a good foundatimon look like?}
\label{sec:good-foundation}

\begin{enumerate}
  \item Support all ``scientifically applicable mathematics''.
  (Really, support the mathemamtics I want to use.)
  \textbf{TODO:} starrt listing problems of interest,
  ...
  the datga to includes completed infinities.
\end{enumerate}

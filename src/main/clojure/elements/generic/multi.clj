    ...
    (let [options   (apply hash-map options)
      ...
      `(let [v# (def ~mm-name)]
         (when-not (and (.hasRoot v#) 
                        (instance? elements.java.generic.MultiFn 
                                   (deref v#)))
           (def ~(with-meta mm-name m)
                (new elements.java.generic.MultiFn 
                     ~(name mm-name) 
                     ~dispatch-fn 
                     ~default)))))))
...
(defmacro defmethod
  ...
  `(. ~(with-meta multifn {:tag 'elements.java.generic.MultiFn}) 
...
(defn remove-all-methods
  ...
  {:added "1.2"
   :static true} 
 [^elements.java.generic.MultiFn multifn]
 (.reset multifn))
...
(defn remove-method
  ...
  {:added "1.0"
   :static true}
 [^elements.java.generic.MultiFn multifn dispatch-val]

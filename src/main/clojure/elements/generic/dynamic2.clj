    ...
    (let [options (apply hash-map options)]
      ...
      `(let [v# (def ~mm-name)]
         (when-not (and (.hasRoot v#) 
                        (instance? elements.java.generic.DynamicFn2 
                                   (deref v#)))
           (def ~(with-meta mm-name m)
                (elements.java.generic.DynamicFn2. 
                  ~(name mm-name))))))))
...
(defmacro defmethod
  ...
  `(.addMethod 
     ~(with-meta dynafn {:tag 'elements.java.generic.DynamicFn2}) 
...
#_(defn remove-all-methods
  ...
  {:added "1.2"
   :static true} 
 [^elements.java.generic.DynamicFn2 dynafn]
 (.reset dynafn))
...
(defn remove-method
  ...
  {:added "1.0"
   :static true}
 [^elements.java.generic.DynamicFn2 dynafn ^Class c0 ^Class c1] 

...
;;----------------------------------------------------------------
(ns elements.scripts.numbers.intervals.bench
  ...
  (:require [clojure.string :as s]
            ...
            [elements.api :as mc]
            [elements.sets.multi :as multi]
            [elements.sets.protocol :as protocol]
  ...
  (:import [org.apache.commons.rng UniformRandomProvider]
           ...
           [elements.java.numbers DoubleInterval]))
;;----------------------------------------------------------------
...
(defn generate-double-intervals [^clojure.lang.IFn generator 
  ...
  (let [gname (mc/fn-name generator)
        ...
        (make-array elements.java.numbers.DoubleInterval n)]
    (dotimes [i n] 
      (aset sets i 
            ^elements.java.numbers.DoubleInterval (generator)))
...
(defn bench [^clojure.lang.IFn f 
  ...
  (let [sets (:sets data-map)
        elements (time (coerce (:raw data-map)))
        dname (.getCanonicalName (class elements))
        ename (.getCanonicalName (mc/element-type elements))
        fname (mc/fn-name f) 
        cname (mc/fn-name coerce)]
    (println fname cname dname ename)
    (try
      (let [result (criterium/benchmark (f sets elements) {})
        ...
        (let [^DoubleInterval seti (aget sets i)
              ...
              (loop [j (int 0)
                ...
                (cond (>= j nelements) 
                      ...
                      (protocol/contains-double? seti (aget elements j)) 
                      (recur (inc j) (inc k))
        ...
        (let [^DoubleInterval seti (aget sets i)
              ...
              (loop [j (int 0)
                ...
                (cond (>= j nelements) 
                      ...
                      (generic/contains-double? seti (aget elements j)) 
                      (recur (inc j) (inc k))
